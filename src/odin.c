...
SEXP model_contents(SEXP internal_p) {
  ...
  SEXP contents = PROTECT(allocVector(VECSXP, 48));
  ...
  SET_VECTOR_ELT(contents, 39, ScalarReal(internal->rT_r_true));
  SET_VECTOR_ELT(contents, 40, ScalarReal(internal->rT_s));
  SET_VECTOR_ELT(contents, 41, ScalarReal(internal->S0));
  SET_VECTOR_ELT(contents, 42, ScalarReal(internal->Sv0));
  SET_VECTOR_ELT(contents, 43, ScalarReal(internal->T_r0));
  SET_VECTOR_ELT(contents, 44, ScalarReal(internal->T_s0));
  SET_VECTOR_ELT(contents, 45, ScalarReal(internal->toff));
  SET_VECTOR_ELT(contents, 46, ScalarReal(internal->ton));
  SET_VECTOR_ELT(contents, 47, ScalarLogical(internal->model_use_dde));
  SEXP nms = PROTECT(allocVector(STRSXP, 48));
  ...
  SET_STRING_ELT(nms, 39, mkChar("rT_r_true"));
  SET_STRING_ELT(nms, 40, mkChar("rT_s"));
  SET_STRING_ELT(nms, 41, mkChar("S0"));
  SET_STRING_ELT(nms, 42, mkChar("Sv0"));
  SET_STRING_ELT(nms, 43, mkChar("T_r0"));
  SET_STRING_ELT(nms, 44, mkChar("T_s0"));
  SET_STRING_ELT(nms, 45, mkChar("toff"));
  SET_STRING_ELT(nms, 46, mkChar("ton"));
  SET_STRING_ELT(nms, 47, mkChar("model_use_dde"));
...
void model_rhs(model_internal* internal, double t, double * state, double * dstatedt, double * output) {
  ...
  double delayed_lambda_v_r_Sv;
  {
    ...
    double lambda_v_r = internal->a * (internal->cA * A_r + internal->cD * D_r + internal->cT * T_r);
